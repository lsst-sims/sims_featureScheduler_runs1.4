...
class Drop_random_detailer(detailers.Base_detailer):
    """Drop some fraction of observations
    """

    def __init__(self, fraction=0.15, seed=42):
        np.random.seed(seed)
    ...
    def __call__(self, observation_list, conditions):
        ...
        cut = np.random.random(len(observation_list))
        keep = np.where(cut >= self.fraction)[0]
        out_observations = [observation_list[indx] for indx in keep]
...
def generate_blobs(nside, nexp=1, exptime=30., filter1s=['u', 'g', 'r', 'i', 'z', 'y'],
    ...
    for filtername, filtername2 in zip(filter1s, filter2s):
        ...
        # Add if a detailer that drops 15% of things from the queue
        detailer_list.append(Drop_random_detailer())
...
def run_sched(surveys, survey_length=365.25, nside=32, fileroot='baseline_', verbose=False,
    ...
    n_visit_limit = None
    observatory = Model_observatory(nside=nside, alt_min=30.)
