...
import os
import argparse
from lsst.sims.featureScheduler.surveys import Deep_drilling_survey, dd_bfs, dd_u_bfs
...
def generate_dd_surveys(nside=None, nexp=2, detailers=None, reward_value=100):
    ...
    surveys.append(Deep_drilling_survey(bfs, RA, dec, sequence='u',
                                        nvis=[4], exptime=60, survey_name=survey_name, reward_value=reward_value, nside=nside,
    ...
    bfs = dd_u_bfs(RA, dec, survey_name, ha_limits)
    surveys.append(Deep_drilling_survey(bfs, RA, dec, sequence='u', nvis=[4], exptime=60,
                                        survey_name=survey_name, reward_value=reward_value, nside=nside,
                                        nexp=nexp, detailers=detailers))
...
def generate_blobs(nside, nexp=1, exptime=30., filter1s=['u', 'g', 'r', 'i', 'z', 'y'],
                   filter2s=['u', 'r', 'i', 'z', 'y', 'y'], pair_time=22.,
    ...
    footprints = standard_goals(nside=nside)
    # Decrease the number of u visits by a factor of 2
    footprints['u'] *= 0.5
    sum_footprints = 0
    for key in footprints:
        sum_footprints += np.sum(footprints[key])
